...
describe('Local Validation: Issue 1 + Issue 2 for Vercel job fixture', () => {
  const vercelJobRequirements = [
    ...
    },
    {
      ...
      importance: 'REQUIRED',
      weight: 1.0,
      extractedValue: 'REST APIs',
      skillSlug: 'rest-api',
      originalText: 'REST APIs',
    },
    ...
    // DOMAIN
    {
      ...
      importance: 'REQUIRED',
      weight: 1.0,
      extractedValue: 'security architecture',
    },
  ];
  ...
  it('Node.js experience: Fastify manifest-only -> CLAIMED/LOW_TRUST', () => {
    const req = vercelJobRequirements.find(
      (r) => r.normalizedCriteria?.experienceType === 'PRACTICAL_DEVELOPMENT'
    );
  ...
  it('Node.js experience: When Fastify has source evidence -> HIGH_TRUST', () => {
    ...
    assert.ok(match.matchConfidence >= 0.7, 'Source-level evidence should have higher confidence');
  });
  ...
  it('TypeScript: VERIFIED evidence-backed -> MATCHED with HIGH_TRUST', () => {
    ...
    const { match } = EvidenceMatchingService._evaluateSkillRequirement(
      tsReq,
      new Map([['typescript', fresherProfile.skills.find((s) => s.slug === 'typescript')]]),
      new Map()
    );
  ...
  it('React: VERIFIED evidence-backed -> MATCHED', () => {
    ...
    const { match } = EvidenceMatchingService._evaluateSkillRequirement(
      reactReq,
      new Map([['react', fresherProfile.skills.find((s) => s.slug === 'react')]]),
      new Map()
    );
  ...
  it('Kubernetes: CLAIMED only -> PARTIAL with LOW_TRUST', () => {
    ...
    const { match } = EvidenceMatchingService._evaluateSkillRequirement(
      k8sReq,
      new Map([['kubernetes', fresherProfile.skills.find((s) => s.slug === 'kubernetes')]]),
      new Map()
    );
  ...
  it('SOAP: no evidence -> MISSING (not inferred from Fastify)', () => {
    ...
    const { match } = EvidenceMatchingService._evaluateSkillRequirement(
      soapReq,
      new Map([['fastify', fresherProfile.skills.find((s) => s.slug === 'fastify')]]),
      new Map()
    );
    assert.equal(match.matchStatus, 'MISSING', 'SOAP must NOT be inferred from Fastify');
  });
});
