...
describe('MCP Registry Contract & Documentation Reconciliation Suite', () => {
  ...
  test('1. Tool Inventory Parity: Live server and Documentation have identical counts (30 tools)', () => {
    assert.equal(
      liveTools.length,
      30,
      `Live registered tools count should be 30, got ${liveTools.length}`
    );
    assert.equal(
      TOOLS_CATALOG.length,
      30,
      `Documented tools count should be 30, got ${TOOLS_CATALOG.length}`
    );
  });
  ...
  test('2. Tool Name Exact Match: Every live registered tool is documented without mismatch or renaming', () => {
    ...
    assert.equal(liveToolNames.size, 30, 'No duplicate tool names in live server');
    assert.equal(docToolNames.size, 30, 'No duplicate tool names in documentation catalog');
