        ...
        evidence: [
          {
            ...
            skillSlug: 'redis',
            skillName: 'Redis',
            sourceLocation: { filePath: 'src/cache.ts' },
            confidenceScore: 1,
          },
          {
            evidenceType: 'CODE_IMPORT_USAGE',
            ...
            sourceLocation: { filePath: 'src/app.ts' },
            confidenceScore: 1,
          },
        ],
      },
        ...
        evidence: [
          {
            ...
            skillSlug: 'nodejs',
            skillName: 'Node.js',
            sourceLocation: { filePath: 'src/worker.ts' },
            confidenceScore: 1,
          },
        ],
      },
...
const jobBackend = {
  ...
  description:
    'Build scalable backend services with TypeScript, PostgreSQL and Redis. Distributed systems experience preferred.',
...
describe('P16-009: Canonical Fact Inventory', () => {
  test('1. project with two redundant descriptions produces one canonical fact', () => {
      ...
      projects: [
        {
          ...
          highlights: [
            'Engineered an automated reporting pipeline in Python for weekly analytics.',
          ],
          technologies: ['Python'],
        },
      ],
    ...
    assert.equal(
      claimFacts.length,
      1,
      `expected 1 merged claim fact, got ${claimFacts.length}: ${JSON.stringify(claimFacts.map((f) => f.text))}`
    );
      ...
      projects: [
        {
          ...
          bullets: [
            ...
            'Configured database migrations with Alembic.',
          ],
          technologies: ['Python'],
        },
      ],
  ...
  test('4. composed bullets cover complementary dimensions (architecture + implementation/reliability)', () => {
    ...
    assert.ok(
      composed.bullets.length >= 2,
      'distinct dimension facts should produce multiple bullets'
    );
  });
  ...
  test('fact extraction is source-order invariant', () => {
      ...
      projects: [
        {
          id: 'p1',
          name: 'A',
          bullets: ['Built feature one with retries.'],
          highlights: ['Designed schema for event storage.'],
        },
    ...
    assert.deepEqual(
      invA.facts.map((f) => f.factId),
      invB.facts.map((f) => f.factId)
    );
  });
  ...
  test('fact fingerprints are deterministic per association', () => {
    assert.equal(
      factFingerprint('Built the service', 'p1'),
      factFingerprint('Built the service', 'p1')
    );
    assert.notEqual(
      factFingerprint('Built the service', 'p1'),
      factFingerprint('Built the service', 'p2')
    );
  });
...
describe('P16-009: Optimizer consumes the canonical inventory', () => {
  test('15. optimizer expands from canonical facts, not raw profile re-discovery', async () => {
    ...
    analyzer.measurePdfBottom = () => ({
      lowestY: 100,
      bottomWhitespacePt: 180,
      pageOccupancyRatio: 0.78,
    });
    ...
    analyzer.computeAcceptanceMetrics = ({ structuredResume }) => ({
      ...
      isSinglePage: true,
      factUtilization: {
        factsRendered: structuredResume.projects.reduce((n, p) => n + p.bullets.length, 0),
        distinctFactsAvailable: 18,
  ...
  test('16. candidate-owned facts are never invented or silently lost during canonicalization', () => {
    ...
    const allAuthoredTexts = candidate.projects.flatMap((p) => [
      ...(p.bullets || []),
      ...(p.highlights || []),
    ]);
  ...
  test('6. DSA with authored candidate evidence renders those bullets verbatim', () => {
    const candidate = syntheticCandidate({
      problemSolving: {
        profileUrl: 'https://leetcode.com/synthetic-candidate',
        bullets: [
          'Practiced graph traversal and dynamic programming patterns weekly across two semesters.',
        ],
      },
    });
    ...
    assert.equal(structuredResume.dsa.hasSection, true);
    assert.deepEqual(structuredResume.dsa.bullets, [
      'Practiced graph traversal and dynamic programming patterns weekly across two semesters.',
    ]);
  });
...
describe('P16-009: Unsupported-claim gating', () => {
  test('11. resume with no candidate metrics renders zero fabricated metrics', () => {
    ...
    assert.ok(
      evidenceValidationReceipt.overallStatus === 'PASS' ||
        evidenceValidationReceipt.overallStatus === 'PASS_WITH_FINDINGS'
    );
  });
  ...
  test('unsupported metric fact is omitted from composition with reason', () => {
    const project = {
      ...
      name: 'Metric Project',
      bullets: [
        'Served 5 million users daily on the platform.',
        'Built the ingestion service for event streams.',
      ],
      technologies: ['Go'],
    };
    const inv = buildCanonicalFactInventory({ projects: [project] });
    ...
    assert.ok(
      omittedReasons.includes('UNSUPPORTED_METRIC'),
      `expected UNSUPPORTED_METRIC omission, got ${JSON.stringify(omittedReasons)}`
    );
...
describe('P16-009: Quality score and gates', () => {
  test('deterministic quality score: identical input → identical score', () => {
    const { structuredResume } = buildStructuredResumeSnapshot({
      candidateProfile: syntheticCandidate(),
      jobPosting: jobBackend,
    });
    const s1 = computeResumeQualityScore({
      structuredResume,
      jobPosting: jobBackend,
      pageCount: 1,
      geometry: { pageOccupancyRatio: 0.9 },
    });
    const s2 = computeResumeQualityScore({
      structuredResume,
      jobPosting: jobBackend,
      pageCount: 1,
      geometry: { pageOccupancyRatio: 0.9 },
  ...
  test('redundant bullets fail the pre-render gate (SEMANTICALLY_REDUNDANT_BULLETS)', () => {
      ...
      projects: [
        {
          name: 'P',
          bullets: [
            'Built a high-throughput distributed telemetry platform.',
            'Engineered a high-throughput distributed telemetry platform.',
          ],
        },
      ],
      experience: [],
    ...
    const finding = gate.findings.find(
      (f) => f.code === GATE_FINDING_CODES.SEMANTICALLY_REDUNDANT_BULLETS
    );
  ...
  test('supported metric (evidence-ref-backed) does not fail the gate', () => {
      ...
      projects: [
        {
          ...
          bullets: [
            {
              text: 'Cut median query latency 40% in load benchmarks.',
              evidenceRefs: [
    ...
    const gate = assessPreRenderQuality({ structuredResume: doc, targetRole: 'Backend Engineer' });
    const finding = gate.findings.find(
      (f) => f.code === GATE_FINDING_CODES.UNSUPPORTED_METRIC_CLAIM
    );
  ...
  test('page-count is a penalty, not the objective (score reacts to occupancy)', () => {
    ...
    const dense = computeResumeQualityScore({
      structuredResume,
      jobPosting: jobBackend,
      pageCount: 1,
      geometry: { pageOccupancyRatio: 0.95 },
    });
    const sparse = computeResumeQualityScore({
      structuredResume,
      jobPosting: jobBackend,
      pageCount: 1,
      geometry: { pageOccupancyRatio: 0.4 },
    });
    assert.ok(
      dense.score > sparse.score,
      `dense (${dense.score}) must outscore sparse (${sparse.score})`
    );
    const twoPages = computeResumeQualityScore({
      structuredResume,
      jobPosting: jobBackend,
      pageCount: 2,
      geometry: { pageOccupancyRatio: 0.9 },
...
describe('P16-009: Snapshot pipeline integrity', () => {
  test('snapshot carries factCompositionReport with traceability', () => {
    const r = buildStructuredResumeSnapshot({
      candidateProfile: syntheticCandidate(),
      jobPosting: jobBackend,
    });
  ...
  test('12. projects with different evidence density receive different bullet budgets', () => {
    ...
    const job = {
      ...
      projectRankings: [
        {
          projectId: 'proj-1',
          projectName: 'Telemetry Platform',
          relevanceScore: 90,
          relevanceBand: 'HIGH',
        ...
        {
          projectId: 'proj-2',
          projectName: 'Retry Queue Service',
          relevanceScore: 70,
  ...
  test('long repository URLs and unknown technologies do not break composition', () => {
    ...
    assert.ok(
      proj.technologies.includes('Zorbglib Framework 9000'),
      'unknown technology must render unchanged'
    );
    assert.equal(contentQualityGate.passed, true);
  });
...
describe('P16-009: Sparse-page handling', () => {
  test('13. sparse page with meaningful omitted evidence flags expansion path', async () => {
    ...
    const claimFactsProj1 = scored.filter(
      (f) => f.association?.projectId === 'proj-1' && f.factType !== 'technology'
    );
    const capacity = determineProjectBulletCapacity({
      claimFacts: claimFactsProj1,
      evidenceCount: 12,
    });
  ...
  test('14. sparse page with NO additional evidence stops cleanly (no filler)', () => {
    ...
    assert.equal(
      contentQualityGate.findings.some(
        (f) => f.code === GATE_FINDING_CODES.WEAK_OPTIONAL_SECTION && f.severity === 'FAIL'
      ),
      false
    );
  });
