...
describe('2. Add Additional Skills via Service (dedicated E2E fixture)', () => {
  ...
  it('adds AWS (Proficient)', async () => {
    ...
    const result = await service.addAdditionalSkill({ tenantId }, candidateId, {
      catalogSkillId: aws[0].id,
      proficiency: 'PROFICIENT',
    });
    awsRecordId = result.id;
    assert.strictEqual(result.provenanceStatus, 'SELF_DECLARED');
    console.log(
      `  ✅ AWS: provenance=${result.provenanceStatus}, proficiency=${result.proficiency}`
    );
  });
  ...
  it('adds Kubernetes (Proficient)', async () => {
    const skill = await db
      .select()
      .from(skillCatalog)
      .where(eq(skillCatalog.slug, 'kubernetes'))
      .limit(1);
    const result = await service.addAdditionalSkill({ tenantId }, candidateId, {
      catalogSkillId: skill[0].id,
      proficiency: 'PROFICIENT',
  ...
  it('adds Redis (Proficient)', async () => {
    const skill = await db
      .select()
      .from(skillCatalog)
      .where(eq(skillCatalog.slug, 'redis'))
      .limit(1);
    const result = await service.addAdditionalSkill({ tenantId }, candidateId, {
      catalogSkillId: skill[0].id,
      proficiency: 'PROFICIENT',
  ...
  it('adds Terraform (Currently Learning)', async () => {
    const skill = await db
      .select()
      .from(skillCatalog)
      .where(eq(skillCatalog.slug, 'terraform'))
      .limit(1);
    const result = await service.addAdditionalSkill({ tenantId }, candidateId, {
      catalogSkillId: skill[0].id,
  ...
  it('all 5 persist on reload', async () => {
    ...
    for (const s of list)
      console.log(`    ${s.skillName}: ${s.provenanceStatus} / ${s.proficiency}`);
  });
  ...
  it('existing evidence-backed skills not downgraded', async () => {
    const evidenceSkills = await db
      .select({ provenance: candidateSkills.provenanceStatus, slug: skills.slug })
