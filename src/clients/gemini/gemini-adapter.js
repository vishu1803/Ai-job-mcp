/**
 * @file Google Gemini AI Provider Adapter (ARCH-026 / ADR-047)
 *
 * Implements the canonical AiProvider interface using the official @google/genai SDK.
 * Encapsulates model routing, task policies, structured JSON schema output, bounded
 * tool calling (max 3 rounds), failure-isolated audit logging, retries, and fallbacks.
 */
...
import { buildGeminiPromptEnvelope } from './gemini-prompt-builder.js';
import { toGeminiResponseSchema, toGeminiTools } from './gemini-schema-converter.js';
import { normalizeGeminiError } from './gemini-error-normalizer.js';
import {
...
/** Approved read/artifact tools permitted for Gemini execution */
export const APPROVED_GEMINI_TOOLS = Object.freeze([
...
export class GeminiProviderAdapter extends AiProvider {
  ...
  constructor(options = {}) {
    ...
    this.sdkClient =
      options.sdkClient || (this.apiKey ? new GoogleGenAI({ apiKey: this.apiKey }) : null);
  ...
  async generateText(rawRequest, options = {}) {
    ...
    if (!client) {
      throw normalizeGeminiError(
        new Error('GEMINI_API_KEY is not configured and no sdkClient was provided.'),
      ...
      try {
        ...
        if (finishReason === 'SAFETY') {
          throw new AiSafetyBlockedError(
            'Gemini blocked generation under safety thresholds.',
        ...
        this.logger.warn(
          { err: lastError, attempt, modelId: activeModelId },
          'Gemini API request attempt failed; evaluating retry/fallback'
        );
        ...
        if (activeModelId !== execConfig.fallbackModelId && execConfig.fallbackModelId) {
          this.logger.warn(
            ...
            'Primary Gemini model exhausted retries; switching to fallback model'
          );
          activeModelId = execConfig.fallbackModelId;
          attempt = 0; // Reset for 1 attempt with fallback
          continue;
        }
      }
    }
  ...
  async generateStructured(rawRequest, options = {}) {
    ...
    try {
      ...
      throw new AiOutputSchemaError(
        'Gemini returned unparseable JSON for structured request.',
    ...
    if (
      ...
      if (!parseResult.success) {
        throw new AiOutputSchemaError(
          'Gemini structured output failed Zod schema contract validation.',
  ...
  async executeToolLoop(rawRequest, options = {}) {
    ...
    while (rounds < maxRounds) {
      ...
      // If no function calls, Gemini returned its final answer
      ...
      for (const fc of functionCalls) {
        ...
        this.logger.info(
          { event: 'ai.tool_call.started', toolName: fc.name, round: rounds },
    ...
    if (!finalResponse) {
      throw new AiToolLoopExhaustedError(
        `Gemini tool execution exceeded maximum turn limit (${maxRounds} rounds).`,
  ...
  async validateHealth() {
    ...
    if (!this.apiKey && !this.sdkClient) {
      return {
        healthy: false,
        latencyMs: 0,
        details: { message: 'GEMINI_API_KEY is not configured.' },
      };
    }
...
export const defaultGeminiAdapter = new GeminiProviderAdapter();
