/**
 * @file Legacy LaTeX Generator Service
 *
 * Extracted legacy fallback path from LatexDocumentGenerator for better maintainability.
 * Preserves exact same behavior and validation as original _generateFromLegacyPackage method.
 */
 ...
 * @param {object} [params.layoutProfile] Computed layout profile from ResumeLayoutEngine
 * @returns {{ texContent: string, candidateName: string, candidateEmail: string, targetRole: string, targetCompany: string }}
 */
export class LegacyLatexGenerator {
  generateFromLegacyPackage({ applicationPackage, candidateProfile, layoutProfile }) {
      ...
      if (Array.isArray(storedProjs) && storedProjs.length > 0) {
        for (const p of storedProjs) {
          ...
          projectsToRender.push({
            ...
            liveUrl:
              p.liveUrl ||
              p.bults?.find((b) => b.includes('vercel.app'))?.match(/https?:\/\/[^\s]+/)?.[0] ||
              null,
