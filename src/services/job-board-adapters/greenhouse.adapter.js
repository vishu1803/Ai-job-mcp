...
function normalizeGreenhouseJob(ghJob, boardToken) {
  ...
  return {
    ...
    applicationUrl: ghJob.absolute_url || `https://boards.greenhouse.io/${boardToken}/jobs/${ghJob.id}`,
...
function inferWorkplaceType(location, content) {
  ...
  if (combined.includes('remote') && !combined.includes('on-site') && !combined.includes('onsite')) {
    return 'REMOTE';
  }
