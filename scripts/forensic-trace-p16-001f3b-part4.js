...
async function main() {
  ...
  const cands = (
    ...
    FROM candidates WHERE id IN (${ids})`)
    )
  ).rows;
  console.log('\n[1] CANDIDATES');
  ...
  const projRows = (
    ...
    FROM projects WHERE candidate_id IN (${ids}) ORDER BY candidate_id, created_at`)
    )
  ).rows;
  ...
  for (const [cid, list] of Object.entries(byCand)) {
    console.log(
      cid.slice(0, 8) + ': ' + JSON.stringify(list.map((x) => ({ name: x.name, status: x.status })))
    );
  }
  ...
  const snaps = (
    ...
    FROM job_analysis_snapshots ORDER BY analyzed_at DESC`)
  ).rows;
      ...
      JSON.stringify(
        {
          ...
          projectRankings: (s.project_rankings || []).map((r) => ({
            n: r.projectName || r.name,
            s: r.relevanceScore,
          })),
          metaTopRelevant: (s.meta_top || []).map((r) => ({
            n: r.projectName || r.name,
            s: r.relevanceScore,
          })),
          metaPortfolioFeatured: ((s.meta_portfolio || {}).featuredProjects || []).map((r) => ({
            n: r.projectName || r.name || r.title,
            s: r.relevanceScore ?? r.score,
          })),
        },
        null,
        1
  ...
  const t24 = (
    ...
    WHERE tenant_id = '24d53f53-780e-4431-b065-32180c354175' ORDER BY created_at`)
  ).rows;
    ...
    for (const a of audits) {
      console.log(
        JSON.stringify({
          action: a.action,
          at: a.created_at,
          meta: a.metadata ? JSON.stringify(a.metadata).slice(0, 200) : null,
        })
      );
    }
  } catch (e) {
