...
async function run() {
  ...
  console.log(
    'All Target.getTargets:',
    tInfo.targetInfos.map((x) => ({ type: x.type, url: x.url, title: x.title }))
  );
  ...
  const evalRes = await cdpSend(ws, 'Runtime.evaluate', {
    expression:
      '({ url: location.href, title: document.title, authStatus: document.getElementById("authStatusText")?.textContent })',
    returnByValue: true,
  });
