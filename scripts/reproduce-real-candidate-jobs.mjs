...
const artifactScratchDir =
  'C:\\Users\\VISHW\\.gemini\\antigravity-ide\\brain\\ea0d5724-0ace-4618-9325-5853f7180381\\scratch';
...
async function runOptimizedJob(label, job, fileKey) {
  ...
  for (const h of optResult.iterationHistory) {
    console.log(
      `  * Iteration ${h.iteration}: pageCount=${h.pageCount}, bottomWhitespace=${h.bottomWhitespacePt}pt, occupancy=${Math.round(h.pageOccupancyRatio * 100)}%, factsRendered=${h.factsRendered}, action="${h.action}"`
    );
  }
...
try {
  ...
  console.log(
    `Metric                              | Job A (Cloudflare)      | Job B (Vercel)          | Job C (Crunchyroll)`
  );
  console.log(
    `------------------------------------+-------------------------+-------------------------+-------------------------`
  );
  ...
  const rows = [
    [
      '1. Target Role Heading',
      resA.optResult.structuredResume.candidateIdentity?.headline,
      resB.optResult.structuredResume.candidateIdentity?.headline,
      resC.optResult.structuredResume.candidateIdentity?.headline,
    ],
    [
      '2. Summary Chars / Sentences',
      `${resA.metrics.summary.chars}c / ${resA.metrics.summary.sentenceCount}s`,
      `${resB.metrics.summary.chars}c / ${resB.metrics.summary.sentenceCount}s`,
      `${resC.metrics.summary.chars}c / ${resC.metrics.summary.sentenceCount}s`,
    ],
    [
      '3. Selected Projects Count',
      `${resA.metrics.projects.count} projects`,
      `${resB.metrics.projects.count} projects`,
      `${resC.metrics.projects.count} projects`,
    ],
    [
      '4. Bullets per Project (R1/R2/R3)',
      resA.metrics.projects.bulletsPerProject
        .map((p) => `${p.name.slice(0, 12)}: ${p.bulletsCount}`)
        .join('; '),
      resB.metrics.projects.bulletsPerProject
        .map((p) => `${p.name.slice(0, 12)}: ${p.bulletsCount}`)
        .join('; '),
      resC.metrics.projects.bulletsPerProject
        .map((p) => `${p.name.slice(0, 12)}: ${p.bulletsCount}`)
        .join('; '),
    ],
    [
      '5. Total Project Bullets',
      `${resA.metrics.projects.totalBullets} bullets`,
      `${resB.metrics.projects.totalBullets} bullets`,
      `${resC.metrics.projects.totalBullets} bullets`,
    ],
    [
      '6. Experience Roles / Bullets',
      `${resA.metrics.experience.rolesCount} roles / ${resA.metrics.experience.bulletsCount} bullets`,
      `${resB.metrics.experience.rolesCount} roles / ${resB.metrics.experience.bulletsCount} bullets`,
      `${resC.metrics.experience.rolesCount} roles / ${resC.metrics.experience.bulletsCount} bullets`,
    ],
    [
      '7. DSA Section Status',
      `${resA.metrics.dsa.rendered ? 'RENDERED' : 'OMITTED (Truthful)'}`,
      `${resB.metrics.dsa.rendered ? 'RENDERED' : 'OMITTED (Truthful)'}`,
      `${resC.metrics.dsa.rendered ? 'RENDERED' : 'OMITTED (Truthful)'}`,
    ],
    [
      '8. Distinct Facts (Avail/Rendered)',
      `${resA.metrics.factUtilization.distinctFactsAvailable} avail / ${resA.metrics.factUtilization.factsRendered} rend`,
      `${resB.metrics.factUtilization.distinctFactsAvailable} avail / ${resB.metrics.factUtilization.factsRendered} rend`,
      `${resC.metrics.factUtilization.distinctFactsAvailable} avail / ${resC.metrics.factUtilization.factsRendered} rend`,
    ],
    [
      '9. Fact Utilization Ratio',
      `${Math.round(resA.metrics.factUtilization.utilizationRatio * 100)}%`,
      `${Math.round(resB.metrics.factUtilization.utilizationRatio * 100)}%`,
      `${Math.round(resC.metrics.factUtilization.utilizationRatio * 100)}%`,
    ],
    [
      '10. Page Count (Strictly 1)',
      `${resA.metrics.pageCount} page (${resA.metrics.isSinglePage ? 'PASS' : 'FAIL'})`,
      `${resB.metrics.pageCount} page (${resB.metrics.isSinglePage ? 'PASS' : 'FAIL'})`,
      `${resC.metrics.pageCount} page (${resC.metrics.isSinglePage ? 'PASS' : 'FAIL'})`,
    ],
    [
      '11. Bottom Whitespace',
      `${resA.metrics.geometry.bottomWhitespacePt} pt`,
      `${resB.metrics.geometry.bottomWhitespacePt} pt`,
      `${resC.metrics.geometry.bottomWhitespacePt} pt`,
    ],
    [
      '12. Page Occupancy Ratio',
      `${resA.metrics.geometry.pageOccupancyPercent}%`,
      `${resB.metrics.geometry.pageOccupancyPercent}%`,
      `${resC.metrics.geometry.pageOccupancyPercent}%`,
    ],
    [
      '13. Pre-Exposure QA Score',
      `${resA.optResult.qaScore}/100 (${resA.optResult.qaPassed ? 'PASS' : 'FAIL'})`,
      `${resB.optResult.qaScore}/100 (${resB.optResult.qaPassed ? 'PASS' : 'FAIL'})`,
      `${resC.optResult.qaScore}/100 (${resC.optResult.qaPassed ? 'PASS' : 'FAIL'})`,
    ],
    [
      '14. Optimizer Iterations Run',
      `${resA.optResult.iterationsRun} iteration(s)`,
      `${resB.optResult.iterationsRun} iteration(s)`,
      `${resC.optResult.iterationsRun} iteration(s)`,
    ],
  ];

  for (const [k, vA, vB, vC] of rows) {
    console.log(
      `${k.padEnd(35)} | ${String(vA).slice(0, 23).padEnd(23)} | ${String(vB).slice(0, 23).padEnd(23)} | ${String(vC).slice(0, 23).padEnd(23)}`
    );
  }
