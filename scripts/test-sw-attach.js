...
async function run() {
  ...
  const attachRes = await cdpSend(ws, 'Target.attachToTarget', {
    targetId: sw.targetId,
    flatten: true,
  });
  console.log('Attached to SW! Session ID:', attachRes.sessionId);

  const evalRes = await cdpSend(
    ws,
    'Runtime.evaluate',
    {
      expression:
        'chrome.windows.create({ url: chrome.runtime.getURL("popup/popup.html"), type: "popup", width: 380, height: 600 }).then(w => ({ id: w.id, tabs: w.tabs }))',
      awaitPromise: true,
      returnByValue: true,
    },
    attachRes.sessionId
  ...
  console.log(
    'All targets after window create:',
    updatedTargets.targetInfos.map((t) => ({ type: t.type, url: t.url }))
  );
