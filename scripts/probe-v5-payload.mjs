...
const pkg = (
  ...
  WHERE application_id = '2f71f4cf-0f86-43eb-b1c0-687e2e2d2d1c' AND version = 5`)
).rows[0];
  ...
  JSON.stringify(
    {
      ...
      targetJobJobFitAnalysis: p.targetJob?.jobFitAnalysis
        ? {
            ...
            first3: (p.targetJob.jobFitAnalysis.projectRankings || []).slice(0, 3).map((r) => ({
        ...
        : 'ABSENT',
      targetJobProjectRankings: p.targetJob?.projectRankings
        ? 'present len=' + p.targetJob.projectRankings.length
        : 'ABSENT',
      ...
      tailoredResumeSelectedProjects: (p.tailoredResume?.selectedProjects || []).map(
        (x) => x.name || x.projectName
      ),
