...
const r = (
  ...
  WHERE metadata::text LIKE '%Built a responsive Next.js 14 React frontend%' LIMIT 5`)
).rows;
...
const r2 = (
  ...
  WHERE excerpt::text LIKE '%Built a responsive Next.js 14 React frontend%'`)
).rows[0];
...
const res = (
  await db.execute(sql`
  SELECT name, metadata::text AS t FROM resources WHERE candidate_id='10a2b51b-09bf-4090-8040-1f60ebeb89c9' AND url ILIKE '%Product-Data-Explorer%'`)
).rows;
for (const x of res) {
  console.log(
    'PDE resource:',
    ...
    '| meta has NestJS:',
    (x.t || '').includes('NestJS'),
    '| meta has Next bullet:',
    (x.t || '').includes('Next.js 14')
  );
