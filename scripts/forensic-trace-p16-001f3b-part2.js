...
const names = (arr) =>
  Array.isArray(arr)
    ? arr.map((p) => p?.name || p?.displayName || p?.projectName).filter(Boolean)
    : null;
...
async function main() {
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          downloadedPdfProjectTitlePresence: found,
          headlineExcerpt: text.slice(0, 200).replace(/\s+/g, ' '),
        },
        null,
        1
      )
    ...
    let tenants = [];
    try {
      tenants = await fs.readdir(storageRoot);
    } catch {
      /* none */
    }
    ...
    for (const tenantDir of tenants) {
      ...
      let keys = [];
      try {
        keys = await fs.readdir(tenantPath);
      } catch {
        continue;
      }
      for (const keyFile of keys) {
        ...
        try {
          const buf = await storage.getDecryptedDocument({
            tenantId: tenantDir,
            storageKey: keyFile.replace(/\.enc$/, ''),
          });
          const h = crypto.createHash('sha256').update(buf).digest('hex');
          if (h === pdfHash) {
            matchFound = true;
            console.log(
              'MATCH: downloaded PDF === stored artifact tenant=' +
                tenantDir +
                ' storageKey=' +
                keyFile
            );
    ...
    if (!matchFound)
      console.log(
        'NO stored artifact matches the downloaded PDF hash (PDF was generated in a previous session/artifact pruned, or modified after download).'
      );
  } catch (e) {
  ...
  for (const z of [
    'handoff-kit-2f71f4cf.zip',
    'handoff-kit-628b7764.zip',
    'handoff-kit-c67e8893.zip',
  ]) {
    try {
      const st = await fs.stat(path.join('.tmp-downloads', z));
      console.log(z + ' modifiedAt=' + st.mtime.toISOString());
    } catch {
      console.log(z + ' (stat failed)');
    }
  }
