...
const names = (arr) =>
  Array.isArray(arr)
    ? arr
        .map((p) => p?.name || p?.displayName || p?.projectName || JSON.stringify(p))
        .filter(Boolean)
    : null;
const short = (s) =>
  typeof s === 'string' && s.length > 24 ? s.slice(0, 12) + '…(' + s.length + ')' : s;
...
async function main() {
  ...
  console.log('\n[1] APPLICATION');
  console.log(
    JSON.stringify({ ...app, metadata: undefined }, (k, v) => (k === 'metadata' ? undefined : v), 2)
  );
      ...
      JSON.stringify(
        {
          ...
          structuredResumeProjectsFull: p.sr_projects?.map((x) => ({
  ...
  const cur =
    pkgs.rows.find((p) => p.lifecycle_state === 'CURRENT') || pkgs.rows[pkgs.rows.length - 1];
    ...
    JSON.stringify(
      {
        ...
        summaryFirst120: (
          payload.structuredResume?.summary?.text ||
        ...
        targetJobJobFitAnalysisAuthoritativeRankings: names(
          payload.targetJob?.jobFitAnalysis?.authoritativeRankings
        ),
        ...
        jobFitAnalysisTopLevel: payload.jobFitAnalysis
          ? {
              source: payload.jobFitAnalysis.source,
              snapshotId: payload.jobFitAnalysis.snapshotId,
              rankings: names(payload.jobFitAnalysis.projectRankings),
            }
          : null,
      },
      null,
  ...
  const kit = app.kit_hash
    ? (
        await db.execute(
          sql`SELECT metadata->'handoffKit' AS kit FROM job_applications WHERE id = ${appId}`
        )
      ).rows[0]?.kit
    : null;
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          DOWNLOADED_PDF_MATCHES_KIT: kitResumeHash
            ? pdfHash === kitResumeHash
            : 'kit hash unavailable',
        },
        null,
        2
      )
    ...
    if (kit?.resume?.texStorageKey && kitResumeHash && pdfHash !== kitResumeHash) {
      console.log(
        'NOTE: downloaded PDF differs from kit resume contentHash — downloaded file may be from a different run.'
      );
    }
  } catch (e) {
  ...
  try {
    if (kit?.resume?.texStorageKey) {
      const texBuf = await storage.getDecryptedDocument({
        tenantId: app.tenant_id,
        storageKey: kit.resume.texStorageKey,
      });
      const tex = texBuf.toString('utf8');
      const projMatches = [
        ...tex.matchAll(/\\textbf\{([^}]+)\}(?=[\s\S]{0,120}?\\atssection|[\s\S]{0,120}?\\href)/g),
      ].map((m) => m[1]);
      const projectSection =
        tex.split('Technical Projects')[1] || tex.split('atssection{Projects}')[1] || '';
      const texProjectNames = [...projectSection.matchAll(/\\textbf\{([^}]+)\}/g)].map((m) => m[1]);
      console.log(
        JSON.stringify(
          {
            texProjectNamesInOrder: texProjectNames,
            summaryInTex: (tex.match(/Professional Summary\}([\s\S]{0,300})/)?.[1] || '')
              .slice(0, 200)
              .replace(/\s+/g, ' '),
          },
          null,
