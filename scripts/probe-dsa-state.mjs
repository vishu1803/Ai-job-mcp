...
console.log(
  JSON.stringify(
    { userCustom: cand.uc_ps, profileMetadata: cand.pm_ps, resumeData: cand.rd_ps },
    null,
    1
  )
);
...
const skills = (
  ...
  WHERE cs.candidate_id = ${CAND}`)
).rows.map((r) => r.name);
console.log(
  '\n[skill names matching DSA]',
  skills
    .filter((n) => /data structures|algorithm|leetcode|binary search|competitive/i.test(n))
    .slice(0, 10)
);
...
const v6 = (
  ...
  FROM application_packages WHERE application_id = '2f71f4cf-0f86-43eb-b1c0-687e2e2d2d1c' AND version = 6`)
).rows[0];
console.log('\n[v6 structuredResume]');
console.log(
  JSON.stringify(
    {
      ...
      experienceFirst: (v6.exp || [])[0]
        ? {
            company: v6.exp[0].company,
            title: v6.exp[0].title,
            bullets: (v6.exp[0].bullets || []).length,
          }
        : null,
      educationCount: (v6.edu || []).length,
      educationFirst: (v6.edu || [])[0]
        ? { institution: v6.edu[0].institution, degree: v6.edu[0].degree }
        : null,
    },
    null,
    1
