...
async function main() {
  ...
  console.log(
    ...
    JSON.stringify(
      reqs
        .slice(0, 4)
        .map((r) => ({ c: r.category, v: r.extractedValue?.slice(0, 40), i: r.importance }))
    )
  );
  ...
  console.log(
    ...
    (fit.matches || []).length,
    'partialMatches:',
    (fit.partialMatches || []).length,
    'missingRequirements:',
    (fit.missingRequirements || []).length,
    'hardBlockers:',
    (fit.hardBlockers || []).length
  );
  ...
  console.log(
    '[4] match_analysis.requirementMatches:',
    (ma.requirementMatches || []).length,
    'skillGaps:',
    (ma.skillGaps || []).length
  );
