...
const pkg = (
  ...
  FROM application_packages WHERE application_id = ${APP} AND version = 5`)
).rows[0];
console.log('[v5 deep]');
console.log(
  JSON.stringify(
...
console.log(
  'topLevelJobFitAnalysis:',
  pkg.top_fit_source,
  pkg.top_fit_snapshot,
  JSON.stringify((pkg.top_fit_rankings || []).map((r) => r.projectName || r.name))
);
...
if (kit.tex_key) {
  ...
  const summary = (tex.match(/Professional Summary\}\s*([\s\S]{0,260})/)?.[1] || '').replace(
    /\s+/g,
    ' '
  );
  console.log('\n[STEP7/8 TEX]');
  console.log(
    JSON.stringify({ headline, texProjects, summaryFirst240: summary.slice(0, 240) }, null, 1)
  );
}
if (kit.pdf_key) {
  ...
  console.log(
    '\n[STEP8 KIT PDF] sha256=' +
      crypto.createHash('sha256').update(pdfBuf).digest('hex').slice(0, 16) +
      '… (kit hash ' +
    ...
    JSON.stringify(
      {
        probes,
        idx,
        pdeFirst: idx.pde >= 0 && (idx.pde < idx.ctm || idx.ctm < 0),
