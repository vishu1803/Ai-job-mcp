...
async function main() {
  ...
  const pkgs = (
    ...
    FROM application_packages WHERE application_id = ${APP_ID} ORDER BY version ASC`)
  ).rows;
      ...
      JSON.stringify(
        {
          ...
          structuredResumeProjects: Array.isArray(p.sr_projects)
            ? p.sr_projects.map((x) => x.name)
            : null,
          targetJobTitle: p.tj_title,
          targetJobProjectRankings: Array.isArray(p.tj_rankings)
            ? p.tj_rankings.map((r) => ({
                n: r.projectName || r.name,
                s: r.relevanceScore,
                band: r.relevanceBand,
              }))
            : p.tj_rankings,
          ...
          targetJobJobFitAnalysis: fit
            ? {
                ...
                rankings: Array.isArray(fit.projectRankings)
                  ? fit.projectRankings.map((r) => ({
                      n: r.projectName || r.name,
                      s: r.relevanceScore,
                      rank: r.relevanceRank,
                    }))
                  : null,
                topRelevant: Array.isArray(fit.topRelevantProjects)
                  ? fit.topRelevantProjects.map((r) => ({
                      n: r.projectName || r.name,
                      s: r.relevanceScore,
                    }))
                  : null,
            ...
            : null,
          tailoredResumeSelectedProjects: Array.isArray(p.tr_sel)
            ? p.tr_sel.map((s) => s.name || s.projectName || s.title)
            : p.tr_sel,
        },
        null,
        2
  ...
  const [cand] = (
    ...
    FROM candidates WHERE id = ${(await db.execute(sql`SELECT candidate_id FROM job_applications WHERE id = ${APP_ID}`)).rows[0].candidate_id}`)
  ).rows;
  ...
  for (const p of profProjects) {
    console.log(
      JSON.stringify({
        name: p.name || p.title,
        id: p.id || p.projectId,
        archived: p.isArchived || p.metadata?.portfolioStatus,
        url: p.repositoryUrl || p.url,
      })
    );
  }
  ...
  try {
    const buf = await storage.getDecryptedDocument({
