...
const cols = (
  await db.execute(
    sql`SELECT column_name FROM information_schema.columns WHERE table_name='projects' ORDER BY ordinal_position`
  )
...
const projRows = (
  await db.execute(
    sql`SELECT candidate_id, id, name, created_at FROM projects ORDER BY candidate_id, created_at`
  )
).rows;
...
const ev = (
  await db.execute(sql`SELECT candidate_id, count(*) FROM evidence_items GROUP BY candidate_id`)
).rows;
