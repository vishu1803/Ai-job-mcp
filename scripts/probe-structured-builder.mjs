...
const UID = (await db.execute(sql`SELECT user_id FROM candidates WHERE id = ${CAND}`)).rows[0]
  .user_id;

// Authoritative rankings from snapshot c45d6bda (the one bound to v5)
const snap = (
  await db.execute(sql`
  SELECT project_rankings FROM job_analysis_snapshots WHERE id = 'c45d6bda-c58d-4a3f-8429-48972d14e224'`)
).rows[0];
const rankings = snap.project_rankings;
console.log(
  '[input] rankings:',
  rankings.map((r) => ({ id: r.projectId, n: r.projectName }))
);
...
const targetJobPosting = {
  ...
  jobFitAnalysis: {
    projectRankings: rankings,
    topRelevantProjects: rankings,
    source: 'EXTENSION_AUTHORITATIVE_SNAPSHOT',
    overallFit: { atsScore: 47, fitBand: 'MODERATE' },
  },
};
  ...
  JSON.stringify(
    {
      ...
      structuredProjects: doc.projects.map((p) => ({
        id: p.projectId,
        name: p.name,
        rank: p.rank,
      })),
...
if (doc.projects.length === 0) {
  ...
  console.log(
    '[diag] rawProjects ids:',
    rawProjects.slice(0, 12).map((p) => p.id || p.projectId || '(none)')
  );
  console.log('[diag] plan ids:', doc.tailoringPlan.selectedProjectIds);
  const map = new Map();
  for (const p of rawProjects) {
    const k = p.id || p.projectId;
    if (k) map.set(k, p);
  }
