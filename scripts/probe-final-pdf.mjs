...
const kit = (
  ...
  FROM job_applications WHERE id = ${APP}`)
).rows[0];
console.log(
  '[kit] pkgHash=' +
...
const tex = (
  await storage.getDecryptedDocument({ tenantId: TENANT, storageKey: kit.tex_key })
).toString('utf8');
const projSection = tex.split(/\atssection\{/).find((s) => /Project/i.test(s.split('}')[0])) || '';
const texProjects = [
  ...(projSection + (projSection.includes('textbf') ? '' : '')).matchAll(/\textbf\{([^}]+)\}/g),
].map((m) => m[1]);
const allTexbf = [...tex.matchAll(/\textbf\{([A-Z][^}]{3,60})\}/g)]
  .map((m) => m[1])
  .filter((n) => /Explorer|Task|Review|Data/i.test(n));
const summary = (tex.match(/Professional Summary\}\s*([\s\S]{0,240})/)?.[1] || '').replace(
  /\s+/g,
  ' '
);
...
const pdfBuf = await storage.getDecryptedDocument({
  tenantId: TENANT,
  storageKey: (
    await db.execute(
      sql`SELECT metadata->'handoffKit'->'resume'->>'storageKey' AS k FROM job_applications WHERE id = ${APP}`
    )
  ).rows[0].k,
});
console.log(
  '[PDF] hash match:',
  crypto.createHash('sha256').update(pdfBuf).digest('hex') === kit.pdf_hash
);
...
const probes = {
  ...
  tailoredHeading:
    /software engineer, data/i.test(text) && !/full-stack & backend developer/i.test(text),
};
  ...
  JSON.stringify(
    {
      probes,
      mentionOrder: idx,
      projectsSectionExcerpt: projectsSection.slice(0, 400).replace(/\s+/g, ' '),
    },
    null,
    1
  )
