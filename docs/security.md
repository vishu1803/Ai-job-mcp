...
## 5. Multi-Tenant Cryptographic & Resource Isolation Model (P2-006)

Antigravity Career Hub enforces multi-tenant isolation as a defense-in-depth, platform-wide architectural boundary:

### 5.1. Immutable Server-Side Authentication Context
* Authentication (`authenticate` middleware) hydrates a strictly frozen context on every request:
  ```javascript
  ...
  ```
* **Anti-Spoofing Barrier**: Tenant context is derived **exclusively** from the server-validated session database record. Client-supplied tenant identifiers (query parameters `?tenant_id=...`, HTTP headers `x-tenant-id`, body parameters `tenant_id`, or URL path segments) are strictly ignored and cannot override or pollute the trusted tenant boundary.

### 5.2. Cross-Tenant Default-Deny (404 NOT_FOUND)
* When a user queries, tests, disconnects, or deletes a resource that does not exist or belongs to another tenant workspace, the system consistently returns **`404 NOT_FOUND`** (`CONNECTION_NOT_FOUND`).
* Cross-tenant lookups never return `403 Forbidden` for ID lookups, preventing attackers from probing UUID existence or enumerating tenant resources across workspace boundaries (IDOR prevention).

### 5.3. Centralized Resource Authorization Helper (`authorizeResourceAccess`)
* Centralized in [`src/security/resource-authorization.js`](file:///src/security/resource-authorization.js):
  * **Tenant Boundary Check**: `resource.tenantId === tenantId` (throws 404 on mismatch).
  * **Read RBAC**: Accessible to `OWNER`, `MEMBER`, and `READONLY`.
  * **Mutation / Deletion RBAC**:
    * `OWNER`: Full tenant-wide mutation authority.
    * `MEMBER`: May only mutate resources they personally created (`resource.userId === user.id`). Non-creator members receive `403 FORBIDDEN`.
    * `READONLY`: Strictly barred from mutating or deleting resources (`403 FORBIDDEN`).

### 5.4. Hardened Repository Guard (`assertTenantId`)
* Every data access method in resource repositories (e.g. [`src/db/repositories/connection.repository.js`](file:///src/db/repositories/connection.repository.js)) executes runtime assertions (`assertTenantId`) ensuring `tenantId` is present, non-empty, and valid.
* All queries strictly enforce composite matching:
  ```sql
  WHERE tenant_id = :trustedTenantId AND id = :requestedResourceId
  ```
* No generic, un-scoped `findById(id)` methods exist for tenant-owned resources.

### 5.5. Trusted Connector Context Derivation
* Connector execution contexts (`ConnectorContext`) can only be minted via [`createTrustedConnectorContext`](file:///src/security/resource-authorization.js) after authentication, tenant isolation validation, and creator/role checks succeed.
* Provider connectors never receive arbitrary user IDs, tenant IDs, or raw request payloads.
