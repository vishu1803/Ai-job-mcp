...
## 6. Backend Responsibilities & Security Boundaries (P2-006)
* **Request Handling & Routing**: Fastify server handling REST endpoints and MCP transport routes.
* **Immutable Multi-Tenant Context (`req.auth`)**: Validating session or Bearer tokens and attaching a frozen context (`userId`, `tenantId`, `sessionId`, `role`). Client-supplied tenant IDs (query, header, body) are strictly ignored.
* **Centralized Resource Authorization (`authorizeResourceAccess`)**: Enforcing tenant boundary (`resource.tenantId === req.auth.tenantId`), RBAC roles (`OWNER`, `MEMBER`, `READONLY`), and creator ownership before any resource service method executes.
* **Cross-Tenant Default Deny (404)**: Mismatched or non-existent tenant resources consistently return `404 NOT_FOUND` to eliminate IDOR and tenant resource enumeration.
* **Repository Guarding (`assertTenantId`)**: Mandating `tenantId` parameter validation on all data access queries to prevent un-scoped SQL executions.
* **Trusted Connector Context (`createTrustedConnectorContext`)**: Minting connector contexts exclusively from server-verified identity and authorized connection records.
* **Schema Validation**: Enforcing Zod input/output schemas on all endpoints and MCP tool arguments.
* **Token Lifecycle Management**: Requesting short-lived GitHub installation tokens (`ghs_*`), rotating refresh tokens, and encrypting tokens at rest via AES-256-GCM.
* **Audit Logging**: Writing immutable records of all data access, MCP tool calls, and external actions via sanitized audit metadata.
