...
export class JobPageDetector {
  ...
  static detect(doc, url) {
    ...
    const JOB_SIGNAL_REGEX =
      /\b(?:we(?:[' ]?re|\s+are)\s+hiring|apply(?:\s+now|\s+for\s+this\s+job)?|submit (?:your )?(?:application|resume)|join our (?:\w+\s+)?team|about the (?:role|opportunity)|job description|requirements?|qualifications?|responsibilities(?: include)?|years (?:of )?experience|full[- ]time|part[- ]time|contract(?:or)? position|benefits(?: package)?|equity|open role|open position|careers?)\b/i;
    ...
    return {
      ...
      seniorityLevel: rawPayload.seniorityLevel || null,
      jobFunction: rawPayload.jobFunction || null,
      industries: rawPayload.industries || null,
      postedAgo: rawPayload.postedAgo || null,
      applicantCount: rawPayload.applicantCount || null,
