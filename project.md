...
| **PHASE 4** | Unified Candidate Data Model | 6 | 6 | 0 | **COMPLETE** | **100.0%** |
...
## 3. Detailed Task Ledger

### PHASE 0: Research and Architecture
*Objective: Define platform boundaries, threat model, candidate data models, and MCP specifications.*

| Task ID | Task Title | Dependencies | Status | Verification Method |
| :--- | :--- | :--- | :--- | :--- |
| **P0-001** | Platform Architecture & Boundaries Review | None | **COMPLETE** | Comprehensive architecture specification document in `docs/architecture.md` covering all 5 architectural layers, security invariants, provider-neutral abstractions, and long-term milestones. |
| **P0-002** | Security Architecture & Threat Modeling | P0-001 | **COMPLETE** | Dedicated security architecture specification in `docs/security.md` covering 10-tier threat model, STRIDE evaluation, multi-tenant boundary defenses, cryptographic key hierarchies, and fail-closed isolation rules. |
| **P0-003** | Unified Candidate & Resource Data Model Design | P0-001, P0-002 | **COMPLETE** | Complete domain data model design in `docs/data-model.md` specifying relational schemas, evidence provenance linkage, and integrity constraints. |
| **P0-004** | Remote MCP Specification & Integration Design | P0-001, P0-003 | **COMPLETE** | Standards-compliant MCP specification in `docs/mcp-server-architecture.md` covering Streamable HTTP 2026-07-28 protocol, tool schemas, and auth flow. |
...
| **P10-001A** | Claude Remote MCP Connector & OAuth 2.1 Architecture & Security Review | P9-006 | **COMPLETE & APPROVED** | Architectural specification `docs/claude-mcp-connector-architecture.md` (`ARCH-037`), `ADR-058` in `docs/decisions.md`. Established Streamable HTTP transport compatibility, OAuth 2.1 Authorization Code Flow with mandatory PKCE S256, RFC 9728 Protected Resource Metadata discovery (`/.well-known/oauth-protected-resource`), RFC 8414 OAuth Authorization Server Metadata discovery (`/.well-known/oauth-authorization-server`), public client registration for Claude Web / Desktop / CLI, immutable token claim identity mapping (`sub`, `tid`, `role`, `scope`), strict role ceiling clamping, zero write bypass / self-approval prevention, public HTTPS perimeter protection (TLS 1.2+, Origin header validation against DNS rebinding), 16-scenario threat model (T-01 to T-16), structured OAuth audit events, and live test strategy against `vishu1803/Ai-job-mcp`. |
| **P10-001** | Configure Claude Remote MCP Custom Connector endpoint compatibility (Public HTTPS, OAuth 2.1) | P7-003, P10-001A | **COMPLETE & VERIFIED** | Implemented OAuth 2.1 Authorization Code Flow with PKCE S256 (`src/services/oauth-authorization.service.js`, `src/routes/oauth.routes.js`, `src/domain/oauth/oauth.schemas.js`), RFC 9728 & RFC 8414 metadata discovery endpoints, Refresh Token Rotation (RTR), token revocation, and dual-auth facade in `src/security/mcp-auth.js`. Applied database migration `drizzle/0005_mushy_the_initiative.sql` creating `oauth_clients`, `oauth_authorization_codes`, and `oauth_tokens` tables. Verified via 17 unit tests (10 in `tests/unit/oauth-authorization-server.test.js`, 7 in `tests/unit/mcp-auth.test.js`), 17 live Fastify+PostgreSQL integration tests (`tests/integration/claude-mcp-connector.test.js`), and full 51-test MCP integration test suite. 100% test pass with 0 DB leaks. |
| **P10-002A** | Claude Free & Pro/Team Multi-Connector Architecture & Compatibility Review | P10-001 | **COMPLETE & APPROVED** | Architectural specification `docs/claude-tier-compatibility-architecture.md` (`ARCH-038`), `ADR-059` in `docs/decisions.md`. Evaluated single-connector (Claude Free) vs multi-connector (Claude Pro/Team) operational characteristics, 9-tool catalog compactness (4.3 KB / ~1,085 tokens, <0.55% context), domain-specific naming collision defenses, cross-connector trust boundaries, and zero backend code changes. |
| **P10-002** | Validate Claude Free (1-connector limit) and Claude Pro/Team multi-connector compatibility | P10-001, P10-002A | **COMPLETE & VERIFIED** | Verified 9-tool catalog self-containment, compact schema sizing (<0.55% context budget), non-interference with external connectors, human-in-the-loop approval enforcement on write operations, and zero backend code changes. |
| **P10-003A** | Provider-Neutral AI Prompt / Tool Response Compatibility Review | P10-002 | **COMPLETE & APPROVED** | Architectural specification `docs/provider-neutral-prompt-compatibility-review.md` (`ARCH-039`), `ADR-060` in `docs/decisions.md`. Verified Inverse Authority Principle, strict domain authoritative computation, transient field normalization boundary, deterministic prompt policy parity, cross-tenant 404 isolation, and designed deterministic integration test suite. |
| **P10-003** | Verify provider-neutral prompt adherence: Claude receives identical tool responses as Gemini | P8-003, P10-002, P10-003A | **COMPLETE & VERIFIED** | Implemented `tests/integration/provider-neutral-tools.test.js`. 8/8 comprehensive integration tests proving 100% bit-for-bit structured response parity between Claude-style Streamable HTTP JSON-RPC and Gemini-style direct dispatch across `get_candidate_profile`, `list_verified_skills`, `inspect_project_evidence`, `analyze_job_fit`, safety ceiling clamping (<=24.9), inverse authority spoofing resistance, multi-tenant 404 isolation, and PromptPolicyRegistry ID/version stability. |
| **P10-004** | Document Claude custom connector setup guide and troubleshooting instructions | P10-002, P10-003 | **COMPLETE & VERIFIED** | Authored `docs/claude-custom-connector-guide.md` covering local tunnel setup (`cloudflared`), environment configuration, GitHub App OAuth callback, Claude UI custom connector setup with `claude-web` Client ID and PKCE S256, complete 9-tool catalog specifications, read-only first verification prompts, two-phase write safety workflows, 12-scenario troubleshooting matrix, and production deployment guide. |
...
| Task ID | Task Title | Dependencies | Status | Verification Method |
| :--- | :--- | :--- | :--- | :--- |
| **P11-001A** | ChatGPT Remote MCP Connector & OAuth 2.1 Architecture Review | P10-004 | **COMPLETE & APPROVED** | Architectural specification `docs/chatgpt-mcp-connector-architecture.md` (`ARCH-040`), `ADR-061` in `docs/decisions.md`. Established Streamable HTTP transport compatibility, OAuth 2.1 client matrix (`chatgpt-web`, `chatgpt-desktop`, `chatgpt-custom-gpt`), PKCE S256 enforcement, RFC 9728/8414 metadata discovery, RFC 8707 resource targeting, and 16-scenario threat model (T-01 to T-16). |
| **P11-001** | Configure ChatGPT Developer Mode MCP connector compatibility (OAuth 2.1, Streamable HTTP) | P7-003, P11-001A | **COMPLETE & VERIFIED** | Preconfigured OAuth clients for ChatGPT in `src/services/oauth-authorization.service.js` with RFC 8252 loopback ports and consent screen client name fallback in `src/routes/oauth.routes.js`. Verified via 28 automated integration tests in `tests/integration/chatgpt-mcp-connector.test.js`. |
| **P11-002A** | ChatGPT Tier Compatibility & Multi-Connector Coexistence Review | P11-001 | **COMPLETE & APPROVED** | Architectural specification `docs/chatgpt-tier-compatibility-architecture.md` (`ARCH-041`), `ADR-062` in `docs/decisions.md`. Verified Plus/Team/Enterprise tier compatibility, tool context economics (9 tools = ~1,035 tokens, <0.85% of 128k context), and multi-action coexistence with zero collisions. |
| **P11-002** | Validate ChatGPT tier context window & multi-connector coexistence | P11-001, P11-002A | **COMPLETE & VERIFIED** | Verified full 9-tool catalog schema discovery over ChatGPT OAuth Bearer token in `tests/integration/chatgpt-mcp-connector.test.js` with compact schema footprint (<1,035 tokens) leaving >99.1% context budget available. |
| **P11-003A** | ChatGPT Consequential Action & Write Safety Architecture Review | P11-002 | **COMPLETE & APPROVED** | Architectural specification `docs/chatgpt-write-safety-architecture.md` (`ARCH-042`), `ADR-063` in `docs/decisions.md`. Established two-phase human confirmation protocol, stopping protocol output, anti-AI self-approval, SHA-256 patch fingerprinting, and dynamic default branch protection. |
| **P11-003** | Verify ChatGPT tool invocation fidelity, schema parsing, and write-safety compliance | P9-005, P11-001, P11-003A | **COMPLETE & VERIFIED** | Implemented and executed live write-safety test cases in `tests/integration/chatgpt-mcp-connector.test.js`: proposal ticket generation with stopping protocol, rejection without `confirmed: true`, Draft PR creation with `confirmed: true`, 403 scope ceiling, 404 multi-tenant isolation, and Refresh Token Rotation (RTR). |
| **P11-004** | Document ChatGPT Developer Mode connector setup guide and troubleshooting matrix | P11-002, P11-003 | **COMPLETE & VERIFIED** | Authored `docs/chatgpt-custom-connector-guide.md` (`GUIDE-004`) covering `cloudflared`/`ngrok` tunnel setup, ChatGPT Custom Actions OAuth 2.1 setup, 9-tool catalog reference, verification prompts, and 6-scenario troubleshooting matrix. |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Google Gemini API** | Streamable HTTP / stdio | API Key / Bearer | YES | YES | YES | **YES (Phase 8 Verified)** | Supports native function calling and MCP schema integration via Gemini SDK. Verified across unit, live Vertex, and integration suites. |
| **Gemini Enterprise** | Streamable HTTP (Remote) | OAuth 2.1 / Bearer | YES | YES | YES | **YES (Phase 8 Verified)** | Remote MCP connector integration verified over Streamable HTTP and Bearer auth. |
| **Claude Free** | Streamable HTTP (Remote) | OAuth 2.1 / HTTPS | YES | YES | YES | **YES (Phase 10 Verified)** | Verified: Free tier supports **one** custom remote MCP connector. 9-tool catalog is 100% self-contained (<0.55% context). |
| **Claude Pro / Team** | Streamable HTTP (Remote) | OAuth 2.1 / HTTPS | YES | YES | YES | **YES (Phase 10 Verified)** | Supports multiple custom remote connectors over public HTTPS endpoint (`claude-web`, `claude-desktop`, `claude-cli`). |
| **ChatGPT Plus / Pro** | Streamable HTTP / SSE | OAuth 2.1 / Bearer | YES | YES | YES | **YES (Phase 11 Verified)** | Developer Mode supports custom MCP servers (read and write tools) with PKCE S256 and RFC 8707 resources. |
| **ChatGPT Team / Enterprise** | Streamable HTTP / SSE | OAuth 2.1 / Bearer | YES | YES | YES | **YES (Phase 11 Verified)** | Workspace-level connector governance and MCP tool support verified across 28 integration tests. |
| **Cursor / Local Agents** | stdio / Streamable HTTP | Local / Bearer | YES | YES | YES | **YES (Phase 7 Verified)** | Native stdio and HTTP support for all registered MCP tools. |
...
| 2026-08-26 | Antigravity AI | v1.1.0 | Completed Phase 10 (Claude Integration): Completed P10-001A through P10-004. Built OAuth 2.1 authorization server facade with PKCE S256, RFC 9728 & RFC 8414 metadata discovery, verified single/multi-connector coexistence (<0.55% context budget), proved 100% bit-for-bit provider-neutral tool response parity, and authored `docs/claude-custom-connector-guide.md`. Verified 48/48 integration tests PASS. |
| 2026-08-26 | Antigravity AI | v1.2.0 | Completed Phase 11 (ChatGPT Integration): Completed P11-001A through P11-004. Verified OpenAI ChatGPT Remote MCP Streamable HTTP compatibility, OAuth 2.1 PKCE S256 parameters (`chatgpt-web`, `chatgpt-desktop` loopback ports), RFC 8707 resource targeting, compact schema context economics (~1,035 tokens, <0.85% context), two-phase write safety human stopping protocols, and authored `docs/chatgpt-custom-connector-guide.md`. Verified 28/28 integration tests PASS and 1,395/1,395 full test suite PASS. |
...
* **P10-002A: CLAUDE FREE / PRO / TEAM MULTI-CONNECTOR ARCHITECTURE REVIEW (Completed & Approved)**:
  * Deliverables Created & Modified:
    * `docs/claude-tier-compatibility-architecture.md`: Comprehensive architectural specification (`ARCH-038`) analyzing:
      * **Claude Free Tier (1-Connector Limit)**: Confirmed the complete 9-tool Career Hub catalog (4 Read, 3 Artifact, 2 Write) is 100% self-contained and sufficient. Free users can execute profile inspection, skill analysis, job matching, resume/cover letter adaptation, portfolio recommendation, and safe PR creation without auxiliary connectors.
      * **Tool Context Token Economics**: Measured total JSON Schema footprint for all 9 tools at **4,338 bytes** (~**1,085 tokens**), consuming <0.55% of Claude's context window.
      * **Claude Pro / Max / Team Multi-Connector Coexistence**: Evaluated cross-connector coexistence (Career Hub + GitHub MCP + Notion MCP). Formulated a 9-tool collision matrix and confirmed existing descriptive names (`get_candidate_profile`, `analyze_job_fit`, `confirm_and_create_pr`) present low collision risk.
      * **Write Safety Non-Interference**: Verified multi-connector configurations cannot bypass two-phase human approval. `confirm_and_create_pr` rejects external patch data and sources code mutations strictly from validated database records.
      * **Cross-Connector Trust Boundary**: Enforced `Connector A != Connector B`. Data from external MCP servers is treated as untrusted user input and sanitized via `SecretScrubber` and Zod validation.
      * **Provider Neutrality**: Reaffirmed Inverse Authority Principle — Claude operates strictly as an interchangeable MCP client with zero database or shell access.
      * **Implementation Decision**: Confirmed **zero backend code changes required** to support both single and multi-connector environments.
    * `docs/decisions.md`: Added `ADR-059` recording the formal architectural acceptance.
...
| **P13.5-005** | Implement MCP Registry Metadata & MCP Apps UI Extension Architecture | P13.5-004 | **COMPLETE & VERIFIED** | Implemented official MCP Registry metadata manifest (`server.json`) conforming to `registry.modelcontextprotocol.io` standard (`2025-12-11` / `2026-07-28`), complete with automated schema and secret leakage validator (`src/mcp/registry/registry-validator.js`) and pre-publication readiness checklist (`docs/mcp-registry-readiness-checklist.md`). Implemented SEP-1865 / `io.modelcontextprotocol/ui` standard with read-only Job Fit Radar MVP UI App (`ui://career-hub/job-fit-radar/v1`), serving sandboxed HTML5 with strict CSP (`connect-src 'none'`), 6-axis SVG radar chart, circular ATS score gauge, and postMessage JSON-RPC bridge while preserving standard structured/text tool fallback when host lacks MCP Apps support. Verified via 17/17 dedicated unit/integration tests and 1,152/1,152 unit regression tests. |
...
* **P13.5-005: IMPLEMENT MCP REGISTRY METADATA & MCP APPS UI EXTENSION ARCHITECTURE (Completed & Verified)**:
  * Deliverables Created & Modified:
    * `server.json` & `src/mcp/registry/server.json`: Official MCP Registry manifest conforming strictly to `https://static.modelcontextprotocol.io/schemas/2025-12-11/server.schema.json` (`ServerDetail`). Declares scoped name (`ai.careerhub/mcp-server`), remote transport (`remotes: [{ type: "streamable-http", url: "https://staging.careerhub.ai/mcp" }]`), `websiteUrl` (`https://staging.careerhub.ai`), `repository` (`source: "github", url: "..."`), and extension & auth metadata in `_meta` (`io.modelcontextprotocol/ui`, `ai.careerhub/auth`, `ai.careerhub/publication`). Status explicitly declared as `BLOCKED UNTIL PUBLIC STAGING`.
    * `src/mcp/registry/registry-validator.js`: Strict Zod validator and automated security checker asserting official `ServerDetail` schema compliance, SemVer compliance, namespace formatting, HTTPS remote endpoints, description length (<=100 chars), rejection of unrecognized root fields, and zero secret/token leakage across all manifest fields.
    * `docs/mcp-registry-readiness-checklist.md`: Operational readiness guide addressing the 15 official registry research questions, DNS TXT verification (`_mcp-registry-verification.careerhub.ai`), and Phase 14 public staging domain prerequisites.
    * `src/mcp/apps/job-fit-radar.app.js`: Interactive Job Fit Radar MVP UI App (`ui://career-hub/job-fit-radar/v1`) with MIME type `text/html;profile=mcp-app`:
      - 100% read-only with zero write authority or repository mutation capabilities.
      - Sandboxed HTML5 with strict inline CSP (`default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src data:;`).
      - Pure SVG 6-axis Radar Chart (Required Skills, Preferred Skills, Project Relevance, Experience Fit, Education Fit, Evidence Confidence) and circular ATS score gauge with dynamic color banding (`#10b981`, `#3b82f6`, `#f59e0b`, `#ef4444`).
      - Verified match chips, missing requirement chips, ranked projects list, and prioritized skill gap remediation cards.
      - SEP-1865 JSON-RPC postMessage bridge (`ui/ready`, `ui/initialize`, `ui/update`) with XSS entity escaping.
    * `src/mcp/apps/index.js`: Registry catalog exporting `JOB_FIT_RADAR_APP_RESOURCE`, `renderJobFitRadarAppHtml`, and `registerCareerMcpApps` server binding helper.
    * `src/domain/mcp/mcp.schemas.js`: Extended `McpToolDefinitionSchema` and `McpToolResultSchema` to support optional `_meta` fields for UI resource linkage.
    * `src/domain/mcp/career-read-tools.schemas.js`: Linked `_meta.ui.resourceUri: 'ui://career-hub/job-fit-radar/v1'` on `analyze_job_fit` and added `ui` validation in `AnalyzeJobFitOutputSchema`.
    * `src/mcp/tools/career-read-tools.js`: Updated `handleAnalyzeJobFit` to return `_meta.ui` envelope.
    * `src/mcp/server.js`: Updated `buildServerInstance` to forward `_meta` during tool registration and tool calls, updated `createCareerMcpServer` to register MCP Apps, and exported `registerCareerMcpApps`.
    * `src/views/mcp-docs.page.js`: Added Section 5 detailing official MCP Registry metadata (`server.json`), MCP Apps architecture (`io.modelcontextprotocol/ui`), Job Fit Radar widget specifications, and Host Compatibility Matrix with explicit evidence levels (`VERIFIED LIVE`, `VERIFIED HERMETICALLY`, `OFFICIAL DOCUMENTATION SUPPORT`, `UNSUPPORTED`).
    * `docs/user-guide.md`: Updated Section 14 Platform Roadmap table with `MCP Apps UI Extensions` as `IMPLEMENTED (JOB-FIT RADAR)` and `Official MCP Registry Listing` as `READY FOR SUBMISSION (NOT PUBLISHED)`.
    * `tests/unit/mcp-registry-metadata.test.js`: 9 unit tests verifying schema validation against official `ServerDetail` specification, `remotes` array transport, `_meta` extension declaration, namespace format, semver validation, max description length, unknown root field rejection, secret leak detection, and HTTPS requirements.
    * `tests/unit/mcp-apps-job-fit-radar.test.js`: 5 unit tests verifying resource URI, MIME type, CSP headers, zero CDN dependencies, SVG radar rendering, and XSS payload escaping.
    * `tests/integration/mcp-apps-extension.test.js`: 5 integration tests verifying resource registration, resource reading, `tools/list` metadata, `tools/call` fallback text preservation, and unauthenticated denial.
  * Quality Gates & Verification:
    * `node --test tests/unit/mcp-registry-metadata.test.js tests/unit/mcp-apps-job-fit-radar.test.js tests/integration/mcp-apps-extension.test.js tests/integration/claude-mcp-connector.test.js tests/integration/chatgpt-mcp-connector.test.js` -> PASS (67/67 tests passing in 68.4s)
    * `npm run test:unit` -> PASS (1,152/1,152 unit tests passing across 295 suites in 31.8s)
    * `npm run lint` -> PASS (0 errors, 0 warnings)
    * `npm run format:check` -> PASS (All matched files Prettier compliant)
    * `npm run db:check` -> PASS (Schema in sync)
    * `git diff --check` -> PASS (0 whitespace errors)
...
* **P14-004: DEPLOY PRODUCTION STAGING INFRASTRUCTURE WITH PERSISTENT CUSTOM DOMAIN & CLOUDFLARE NAMED TUNNEL (In Progress - Local & Cloudflare Staging Configuration Verified)**:
  * Deliverables Created & Modified:
    * `docs/cloudflare-staging-architecture.md`: Comprehensive architectural specification (`ARCH-054`) and operational runbook defining:
      1. End-to-end network topology: Public Internet -> Cloudflare Edge TLS (`aicareershub.tech`) -> Persistent Named Tunnel (`career-hub-dev`) -> Localhost `cloudflared` -> Fastify application (`http://127.0.0.1:3000`) -> Aiven PostgreSQL 17.
      2. Domain structure & DNS routing: `dev.aicareershub.tech` CNAME `<TUNNEL_UUID>.cfargotunnel.com`.
    ...
    * `src/config/env.js`: Enhanced environment loader to support explicit configuration switching via `ENV_FILE` and `APP_ENV` (`.env.staging.local`) while preserving `.env.local` default for local development.
    * `tests/unit/env-config.test.js`: Added 4 unit tests verifying local defaults, staging variable schemas, production key enforcement, and 64-hex/44-b64 master key validation.
    * `tests/integration/staging-proxy-security.test.js`: Dedicated 8-test security integration test suite verifying:
      ...
      6. RFC 9728 Protected Resource Metadata & RFC 8414 Authorization Server Metadata discovery endpoint payloads on `dev.aicareershub.tech`.
    ...
    * `tests/integration/auth.test.js`: Verified end-to-end OAuth 2.1 flow with isolated rate limiting across 18 test scenarios (18/18 PASS).
    * `tests/integration/web-application-routes.test.js`: Verified web application routes with candidate identity parity and AST code evidence citations (20/20 PASS).
    * `.gitignore`: Added targeted `.cloudflared/` rule under Credentials & Key Files to prevent repository-local credential commits.
    * `.env.example`: Updated staging domain configuration examples (`https://dev.aicareershub.tech`) and metadata URL documentation.
  * Quality Gates & Verification:
...
* **P14-004D: MCP PRODUCTION CONFORMANCE GAP REMEDIATION, RESUME/PROVENANCE CORRECTION & END-TO-END PUBLIC ACCEPTANCE (Complete & Verified)**:
  ...
  * Quality Gates & Verification:
    ...
    * `npm run db:check` -> PASS (Schema in sync)
  * Status: **`COMPLETE & VERIFIED`**.
* **P14-005G: AST INGESTION PRODUCTION UX: LOADING, PROGRESS, BUTTON LOCKING & STATE-SAFE INGESTION (Step 1F - Complete & Verified)**:
  * Deliverables Created & Modified:
    * `src/services/ingestion-state.service.js`: Dedicated multi-tenant Ingestion State Machine service implementing run states (`IDLE`, `QUEUED`, `RUNNING`, `COMPLETED`, `PARTIAL_FAILURE`, `FAILED`, `CANCELLED`) and repository states (`QUEUED`, `RUNNING`, `COMPLETED`, `FAILED`), unique `ingestionRunId` UUID generation, immutable selected repository snapshotting, concurrency idempotency lock rejecting concurrent executions (`409 Conflict`), and credential-scrubbed safe metadata serialization.
    * `src/services/candidate-repository-ingestion.service.js`: Enhanced `syncCandidateRepositories` with `onProgress` lifecycle callback dispatching `REPOSITORY_STARTED`, `PHASE_CHANGED`, `REPOSITORY_COMPLETED`, and `REPOSITORY_FAILED` events.
    * `src/routes/web.routes.js`: Updated `GET /onboarding` (Step 4) to reconstruct server run state for reload/multi-tab consistency, `POST /onboarding/sync` (returns `202 Accepted` + triggers async pipeline with progress listeners and structured audit logs `ingestion.started`, `ingestion.repository_started`, `ingestion.repository_completed`, `ingestion.completed`), `GET /onboarding/ingestion/status` (lightweight polling endpoint for live metadata), and `POST /onboarding/ingestion/retry` (resets run state for clean restart).
* **P14-005H: UNIFY RESUME QUALIFICATIONS + GITHUB EVIDENCE INTO CANONICAL CAREER PROFILE (Step 1G - Complete & Verified)**:
  * Deliverables Created & Modified:
    * `src/services/source-resume-ingestion.service.js`: Enhanced `reviewAndApproveResume` to propagate complete approved resume qualifications into `candidates.profileMetadata.resumeData` (contact info, identity, structured work experience with dates and bullets, structured education with degree and institution, projects with tech stack and bullets, certifications, and skills), while leaving unapproved raw uploads isolated.
    * `src/services/candidate-profile.service.js`: Unified qualification precedence and truth status computation across `getCareerProfile` and `_extractResumeDataFromSections`. Enforced explicit user edits (`userCustom`) taking precedence over resume claims (`resumeData`), cross-referenced resume claims with AST repository evidence to calculate composite skill truth status (`VERIFIED` vs `CLAIMED`), marked AST projects matching resume projects as `CORROBORATED`, preserved unverified resume projects as `CLAIMED`, and decoupled `profileReadiness` from `jobSearchReadiness` metrics.
    * `tests/unit/candidate-career-profile.test.js`: Added chainable Drizzle ORM mock query builder and 5 new comprehensive unit test scenarios (Scenarios 19–23) verifying write-on-approve resume qualification propagation, user-custom overlay precedence, composite skill truth status derivation, AST project corroboration, and independent profile readiness scoring.
    * `tests/integration/web-application-routes.test.js` & `tests/integration/penetration-testing.test.js`: Hardened test harnesses against external network calls and parallel execution concurrency.
  * Quality Gates & Verification:
    * `node --test tests/unit/candidate-career-profile.test.js tests/unit/candidate-profile.service.test.js` -> PASS (32/32 tests passing)
    * `npm run test:unit` -> PASS (1,379/1,379 unit tests passing across 353 suites)
    * `node --test tests/integration/source-resume-ingestion.test.js` -> PASS (10/10 tests passing)
    * `node --test tests/integration/career-profile-and-compliance.test.js` -> PASS (6/6 tests passing)
    * `node --test tests/integration/web-application-routes.test.js` -> PASS (20/20 tests passing)
    * `node --test tests/integration/penetration-testing.test.js` -> PASS (40/40 tests passing)
    * `npm run lint` -> PASS (0 errors, 0 warnings across entire codebase)
    * `npm run format:check` -> PASS (100% Prettier compliant)
    * `npm run scan:secrets` -> PASS (0 exposed secrets detected)
    * `npm run db:check` -> PASS (Schema in sync)
  * Status: **`COMPLETE & VERIFIED`**.
* **P14-005I: CAREER PROFILE TRUTH MODEL AUDIT: SKILLS + PROJECTS + RESUME/GITHUB CORROBORATION (Step 1H - Complete & Verified)**:
  * Deliverables Created & Modified:
    * `src/domain/career/skill-taxonomy.js`: Implemented fine-grained category classification (`SkillTaxonomyEngine.classifyCategory`) mapping technical skills to `CORE_LANGUAGE`, `FRAMEWORK`, `DATABASE`, `PROTOCOL`, `PLATFORM`, `CLOUD`, `AI_ML`, `TOOL`, `LIBRARY`, `DEPENDENCY_SIGNAL`, `ARCHITECTURE`, `CONCEPT`, and `OTHER`. Implemented tier classification (`classifyTier`) separating `PRIMARY` career competencies from low-level `SIGNAL` dependencies/middleware.
    * `src/domain/candidate/career-preferences.schemas.js`: Enriched `CandidateCareerProfileSchema` with `primarySkills` (top-level competencies), `technologySignals` (underlying libraries and implementation signals), `fineCategory`, `tier` (`PRIMARY` | `SIGNAL`), and `source` (`GITHUB`, `RESUME`, `BOTH`).
    * `src/services/candidate-profile.service.js`: Audited and overhauled `getCareerProfile` truth model: normalized canonical skill variants (`Postgresql`/`PostgreSQL` -> `PostgreSQL`, `Node`/`NodeJS` -> `Node.js`), preserved ALL 159+ underlying evidence items and skills without loss, structured partitioned outputs for primary career skills vs implementation signals, implemented multi-factor project corroboration (name similarity, URLs, technology stack overlap), preserved unmatched resume projects as `CLAIMED`, and decoupled `profileReadiness` (100% complete identity & qualifications) from `completeness` (job search matching criteria).
    * `src/views/profile.page.js`: Upgraded Career Profile Web UI rendering separated `Career Profile: 100% Populated` banner alongside `Job Search Discovery Matching` configuration card, prominent Primary Career Skills with clear badges (`✓ [Corroborated]`, `✓ [Verified]`, `[Claimed]`), collapsible Technology & Implementation Signals section with evidence signal counts, and Corroborated Highlighted Projects.
    * `tests/unit/candidate-career-profile.test.js`: Added 5 new unit test scenarios (Scenarios 24–27, total 27 scenarios) verifying dependency tier classification, canonical variant deduplication, multi-factor project matching, and HTML Profile view rendering parity.
  * Quality Gates & Verification:
    * `node --test tests/unit/candidate-career-profile.test.js tests/unit/skill-taxonomy.test.js` -> PASS (63/63 tests passing)
    * `npm run test:unit` -> PASS (1,383/1,383 unit tests passing across 353 suites)
    * `node --test tests/integration/web-application-routes.test.js` -> PASS (20/20 tests passing)
    * `npm run lint` -> PASS (0 errors, 0 warnings across entire codebase)
    * `npm run format:check` -> PASS (100% Prettier compliant)
    * `npm run db:check` -> PASS (Schema in sync)
  * Status: **`COMPLETE & VERIFIED`**.
* **P14-005J: FINAL SKILL TAXONOMY CORRECTION: PRIMARY CAREER SKILLS vs TECHNOLOGY / IMPLEMENTATION SIGNALS (Step 1I - Complete & Verified)**:
  * Deliverables Created & Modified:
    * `src/domain/career/skill-taxonomy.js`: Overhauled `SkillTaxonomyEngine.classifyCategory` and `classifyTier` with comprehensive semantic classification policy separating core career categories (`CORE_LANGUAGE`, `FRAMEWORK`, `DATABASE`, `PROTOCOL`, `PLATFORM`, `CLOUD`, `AI_ML`, `TOOL`) from supporting implementation signals (`LIBRARY`, `UI_COMPONENT`, `UTILITY_PACKAGE`, `DEV_HELPER`, `BUILT_IN_MODULE`, `DEPENDENCY_SIGNAL`). Added `getPrimarySkillRank` enforcing recruiter-focused priority ordering (Languages -> Backend Frameworks -> Frontend Frameworks -> Databases -> Protocols -> Platforms -> Cloud -> AI/ML -> Major Tools).
    * `src/services/candidate-profile.service.js`: Integrated semantic ranking into `getCareerProfile` sorting for `primarySkills` (Domain Category Rank -> Truth Status -> Confidence Score -> Alphabetical), preserved all 159+ skills and underlying evidence in `technologySignals`, maintained strict decoupled truth status (`VERIFIED` vs `CLAIMED`), preserved resume-only skills as `CLAIMED`, and maintained complete project technology preservation on project cards.
    * `tests/unit/skill-taxonomy.test.js`: Added Section 11 (11 new test scenarios) verifying semantic classification for React (`PRIMARY`), React Dialog/UI components (`SIGNAL UI_COMPONENT`), Node.js (`PRIMARY PLATFORM`), Node Dns/built-ins (`SIGNAL BUILT_IN_MODULE`), Python (`PRIMARY CORE_LANGUAGE`), Python Dotenv/utilities (`SIGNAL UTILITY_PACKAGE`), Fastify (`PRIMARY FRAMEWORK`), Docker (`PRIMARY CLOUD`), Docker Compose (`SIGNAL TOOL`), React Query vs React Query Devtools, and deterministic primary skill category priority rank order.
    * `tests/unit/candidate-career-profile.test.js`: Added Scenarios 28–32 verifying 4-way tier/truth status permutations (`PRIMARY+VERIFIED`, `PRIMARY+CLAIMED`, `SIGNAL+VERIFIED`, `SIGNAL+CLAIMED`), strict `CLAIMED` retention for resume-only skills, GitHub corroboration, component package signal tiering, and schema/MCP validation.
  * Quality Gates & Verification:
    * `node --test tests/unit/candidate-career-profile.test.js tests/unit/skill-taxonomy.test.js` -> PASS (79/79 tests passing)
    * `npm run test:unit` -> PASS (1,399/1,399 tests passing across 354 suites)
...
* **P14-005M: RESUME SEMANTIC NORMALIZATION, MULTI-STAGE ENTITY RESOLUTION, SCOPE ATTRIBUTION & RELATIONSHIP MAPPING (Complete & Verified)**:
  ...
  * Quality Gates & Verification:
    ...
    * `npm run format:check` -> PASS (100% Prettier compliant)
    * `npm run scan:secrets` -> PASS (0 exposed secrets detected)
* **P14-005N: CAREER PROFILE SAAS REDESIGN & INTELLIGENT UX SETUP (Complete & Verified)**:
  * Deliverables Created & Modified:
    * `src/views/profile.page.js`: Completely redesigned the user-facing Career Profile page with Linear/Ashby production aesthetic. Implemented compact profile completeness card (78% completion bar, 4 section status pills `✓ Identity`, `✓ Skills & Evidence`, `! Preferences`, `○ Eligibility`), 1-click AI Profile Suggestions quick-fill drawer, categorized primary skills (Core Languages, Backend & APIs, Frontend, Databases, Cloud & DevOps, AI/ML, Tools & Platforms) with subtle evidence badges (`✓ Verified`, `✓ Corroborated`, `○ Self-reported`), collapsible Technology & Implementation Signals details with evidence counts, redesigned uniform project cards with interactive filter tabs (`All`, `Verified`, `GitHub`, `Resume`), searchable multi-select chip inputs with suggestions for Target Roles, Locations, Tech Stack, and Work Auth, toggleable Industry chips, segmented Remote & Relocation controls, sticky save action bar with unsaved change detection, and toast notification.
  * Quality Gates & Verification:
    * `node --test tests/unit/candidate-career-profile.test.js` -> PASS (41/41 test scenarios passing)
    * `node --test tests/integration/web-application-routes.test.js` -> PASS (20/20 integration tests passing)
    * `npm run test:unit` -> PASS (1,435/1,435 master unit tests passing across 361 suites)
    * `npm run test:db-lifecycle-check` -> PASS (59 DB test files verified compliant, 0 leaks)
    * `npm run lint` -> PASS (0 errors, 0 warnings across entire codebase)
    * `npm run format:check` -> PASS (100% Prettier compliant)
...
| **P14-005** | Implement Automated Database Backup, Disaster Recovery Runbook & Metrics | P14-004D | **COMPLETE & VERIFIED** | Architectural specifications `docs/disaster-recovery.md` (`ARCH-055`), `docs/backup-architecture.md` (`ARCH-056`), `docs/operational-runbook.md` (`ARCH-057`), and `ADR-072`. **Currently Complete**: Aiven managed daily base backups + continuous WAL archiving + 10-day PITR, PITR/fork recovery procedure, local logical backup export with AES-256-GCM envelope encryption & SHA-256 manifest (`src/services/backup-export.service.js`, `scripts/backup-export.js`), encrypted document storage snapshotting (`storage/documents/`), automated restore drill (`scripts/test-dr-restore.js`, `tests/integration/disaster-recovery.test.js` restoring 24 tables, 1,929 rows, 44 document blobs, measured RTO = 20.48s, RPO <= 5m, 100% bit parity), DB cleanup verification (0 orphan DBs), Prometheus operational metrics (`src/monitoring/metrics.service.js`, `GET /metrics`), and DR runbook. **Deferred to Pre-Production Launch**: Production offsite S3/R2 backup bucket, production key escrow integration, automated scheduled offsite upload, independent provider-loss disaster drill, and production cross-region document replication (Rationale: current priority is validating the Career Hub MCP/application product; Aiven managed backup/PITR is sufficient for staging). 1,302 unit tests (100%), 0 lint errors, 0 secrets. |
| **P14-005B** | Career Profile Completeness, Resume-to-Profile Ingestion & Canonical Profile Model (Step 1) | P14-005 | **COMPLETE & VERIFIED** | Resolved profile model vs MCP surface disparity. Implemented unified canonical Career Profile across Web UI (`/profile`), MCP tools (`get_candidate_profile`, `get_career_profile`), and MCP resource (`career://profile`). Added `ProfileCompletenessSchema` with actionable status (`COMPLETE FOR JOB SEARCH` vs `NEEDS ATTENTION`), enriched `CandidateCareerProfileSchema`, zero-inference explicit preference extraction (`extractExplicitPreferences`), voluntary eligibility modeling (`workAuthorization`, `visaSponsorshipRequired`, `availabilityDate`), preserved AST vs self-reported truth separation (`VERIFIED`, `CLAIMED`, `USER_PROVIDED`), and one-time search query overrides. 18 dedicated test scenarios in `tests/unit/candidate-career-profile.test.js` (18/18 PASS), 1,320/1,320 total unit tests passing across 348 suites, Prettier clean, ESLint clean, 0 exposed secrets. |
| **P14-005C** | GitHub App Complete Repository Access Audit & Configuration (Step 1A) | P14-005B | **COMPLETE & VERIFIED** | Audited live GitHub App installation metadata (App ID `4670662`, Installation ID `155430459`, Account `vishu1803`), verified `repository_selection: "all"` active mode, confirmed 41 total visible repositories (38 public, 3 private), confirmed least-privilege permissions (`contents: read`, `metadata: read` only, zero administrative/destructive grants), audited synchronized vs available repositories (1 active repo `Ai-job-mcp`, 40 repos available for user-consented selection), confirmed strict separation of repository access from evidence verification, preserved all claimed unverified projects, increased Web UI repository listing pagination limit to 100 in `src/routes/web.routes.js`, verified default-deny 404 on unauthorized repo IDs, and verified cross-tenant isolation. 10 dedicated test scenarios in `tests/unit/github-repository-access-audit.test.js` (10/10 PASS), 1,330/1,330 unit tests passing (100%), 0 lint errors, 0 secrets. |
| **P14-005D** | Onboarding Step-3 Repository Discovery & Multi-Selection Filtering (Step 1B) | P14-005C | **COMPLETE & VERIFIED** | Fixed `listResources` parameter signature (`credentials = { installationId }`, `options = { limit: 100 }`), updated `GET /onboarding` and `POST /onboarding/repositories/select` in `src/routes/web.routes.js` with server-side repository validation against GitHub App installation, rendered all 41 accessible repositories in Step 3 UI with instant search, filter pills (`All`, `Available`, `Indexed`, `Public`, `Private`), quick actions (`Select All Available`, `Deselect All`), dynamic selection counter, and status badges (`INDEXED` vs `AVAILABLE`). 9 dedicated unit tests in `tests/unit/onboarding-repository-selection.test.js` (9/9 PASS), 1,339/1,339 total unit tests passing across 350 suites, Prettier clean, ESLint clean, 0 secrets. |
